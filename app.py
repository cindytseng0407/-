...
def main():
    ...
    with col_input:
        ...
        with st.expander("📐 面積與格局", expanded=True):
            ...
            # 依商圈顯示車位行情
            district_parking = PARKING_PRICE_BY_DISTRICT.get(district, {})
            if district_parking:
                parking_rows = "".join([
                    f"{ptype}：<b>{price//10000}萬</b><br>"
                    for ptype, price in district_parking.items()
                ])
            else:
                parking_rows = "各類型車位行情請洽建商"
            st.markdown(f"""
            <div style='background:#f0f7ff; border-left:4px solid #5b8cff;
                ...
                <div style='font-size:0.75rem; color:#6B7280; margin-top:6px;'>
                    ※ 車位於實價登錄中獨立登記，本系統估算不含車位，請另行參考上述行情。
                </div>
            </div>
            """, unsafe_allow_html=True)
